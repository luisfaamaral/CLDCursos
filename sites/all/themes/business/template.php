...
function business_preprocess_block(&$variables) {
  global $user;

  if($variables['block']->delta == 'user-menu' ) {
    global $user;
    $variables['block']->subject = "Bem Vindo ". $user->name;
  }
  if ( $variables['block']->delta == 'atualizacoes-block' ) {
    $variables = str_replace("@cadastro", "<a class='cadastro' href='curso/pre-inscricao/@nid'>Pré-Inscrição</a>", $variables);
  }
}
